...
const ContactInfoSection: React.FC<ContactInfoSectionProps> = ({
  ...
  const [customerName, setCustomerName] = useState(
    initialDetails.customerName || "",
  );
  const [customerPhone, setCustomerPhone] = useState(
    initialDetails.customerPhone || "",
  );
  const [customerEmail, setCustomerEmail] = useState(
    initialDetails.customerEmail || "",
  );
  const [emailError, setEmailError] = useState<string | null>(null);
  const [selectedCountry, setSelectedCountry] = useState(
    initialDetails.selectedCountry || "",
  );
  const [countries, setCountries] = useState<Country[]>([]);
  const [countriesLoading, setCountriesLoading] = useState(true);
  const [selectedRegion, setSelectedRegion] = useState(
    initialDetails.selectedRegion || "",
  );
  ...
  useEffect(() => {
    ...
    const handler = setTimeout(() => {
      ...
      const countryName =
        selectedCountryObj?.englishName ||
        selectedCountryObj?.native ||
        selectedCountry;
      // --- End Calculations ---

      // Find the selected region name
      const selectedRegionName =
        italianRegions.find((r) => r.id.toString() === selectedRegion)?.name ||
        selectedRegion;
            ...
            <SelectContent>
              {countriesLoading ? (
                <SelectItem value="loading" disabled>
                  {t("contactInfo.loadingCountries")}
                </SelectItem>
              ) : countries.length === 0 ? (
                <SelectItem value="error" disabled>
                  {t("contactInfo.errorLoadingCountries")}
                </SelectItem>
              ) : (
                countries.map((country) => (
                  <SelectItem key={country.code} value={country.code}>
                    {country.native}{" "}
                    {country.englishName &&
                    country.native !== country.englishName
                      ? `(${country.englishName})`
                      : ""}
                  </SelectItem>
                ))
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                {regionsLoading ? (
                  <SelectItem value="loading" disabled>
                    {t("contactInfo.loadingRegions")}
                  </SelectItem>
                ) : italianRegions.length === 0 ? (
                  <SelectItem value="error" disabled>
                    {t("contactInfo.errorLoadingRegions")}
                  </SelectItem>
                ) : (
