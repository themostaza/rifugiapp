...
async function checkBlockedDays({
  checkIn,
  checkOut,
}: DateRange): Promise<BlockedDay[]> {
...
async function checkOngoingBookings(
  ...
  console.log(
    `🔍 Checking ongoing bookings for session: ${currentSessionId || "(no session)"}`,
  );

  console.log("📅 Checking ongoing bookings for date range:", {
    checkIn,
    checkOut,
  });
  ...
  const ongoingBookingsFromOtherSessions = (allOngoingBookings || []).filter(
    (booking) => {
      const isOwnSession =
        currentSessionId && booking.session_id === currentSessionId;
      if (isOwnSession) {
        console.log(
          `Ignoring booking hold ID ${booking.id} as it belongs to the current session.`,
        );
      }
  ...
  console.log(
    `⚠️ Found ${ongoingBookingsFromOtherSessions.length} overlapping booking(s) from other sessions. Checking expiry...`,
  );

  // Controlla se ALMENO UNO dei blocchi da altre sessioni è ancora valido (non scaduto)
  const hasValidOngoingBookingFromOthers =
    ongoingBookingsFromOtherSessions.some((booking: BookingOnHold) => {
      const timeIsUpAt = new Date(booking.time_is_up_at);
      const enteredPayment = booking.entered_payment
        ? new Date(booking.entered_payment)
        : null;
      const paymentTimeLimit = enteredPayment
        ? new Date(enteredPayment.getTime() + 7 * 60 * 1000)
        : null; // +7 minutes

      const isValid =
        booking.still_on_hold &&
        (timeIsUpAt > nowCET ||
          (enteredPayment && paymentTimeLimit && paymentTimeLimit > nowCET));

      if (isValid) {
        console.log(
          `Blocking due to valid hold ID ${booking.id} from session ${booking.session_id || "(unknown)"}`,
        );
      }
      return isValid;
    });
  ...
  console.log(
    `✅ Overlapping bookings check result (from others): ${hasValidOngoingBookingFromOthers}`,
  );
...
async function checkBedAvailability(params: SearchParams) {
  ...
  const { data: rawBeds, error: bedsError } = await supabase.from("RoomLinkBed")
    .select(`
    id,
    roomId,
    name,
  ...
  existingReservations?.forEach((reservation) => {
    ...
    console.log(
      `📅 Processing reservation dates for reservation ID ${reservation.id}:`,
      reservationDates,
    );
      ...
      reservation.RoomReservation?.forEach((roomRes) => {
        ...
        roomRes.RoomReservationSpec?.forEach((spec) => {
          if (spec.roomLinkBedId) {
            ...
            console.log(
              `  🛌 Bed booked via Spec ID ${spec.id}: roomLinkBedId=${spec.roomLinkBedId} for date ${dateStr}`,
            );
        ...
        roomRes.ReservationLinkBedBlock?.forEach((block) => {
          if (block.day === dateStr && Array.isArray(block.roomLinkBedId)) {
            ...
            console.log(
              `  🚫 Beds blocked via Block ID ${block.id} for date ${block.day}: roomLinkBedIds=[${block.roomLinkBedId.join(", ")}]`,
            );
  ...
  datesInRange.forEach((dateStr) => {
    ...
    console.log(
      `📅 Date ${dateStr}: ${Array.from(occupiedBeds).length} occupied beds`,
    );
  });

  // Calculate total required beds
  const totalRequiredBeds = params.guests.reduce(
  ...
  console.log(
    "✅ Available beds for the entire stay (IDs):",
    availableBeds.map((b) => b.id),
  );

  console.log(
    `📅 Found ${availableBeds.length} beds available for all dates in range ${params.checkIn} to ${params.checkOut}`,
  );
  ...
  const availabilityByNightAndRoom = datesInRange.map((dateStr) => {
    ...
    allBeds?.forEach((bed) => {
      if (!roomAvailabilityMap.has(bed.roomId)) {
        // Sort images by displayOrder (ascending), then by createdAt (descending) as fallback
        const sortedImages = bed.Room.RoomImage
          ? [...bed.Room.RoomImage]
              .sort((a, b) => {
