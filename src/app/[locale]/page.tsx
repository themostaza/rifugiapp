...
const RoomList: React.FC<
  RoomListProps & { t: (key: string, vars?: Record<string, unknown>) => string }
> = ({
  rooms,
  onSelect,
  totalGuests,
  ...
  const getTotalUnassigned = () => {
    ...
    return unassigned.adults + unassigned.children + unassigned.infants;
  };

  const getTotalGuests = () => {
    return totalGuests.adults + totalGuests.children + totalGuests.infants;
  };
  ...
  return (
    <div className="space-y-4">
      <div
        className="
          sticky top-0 z-30
          bg-gradient-to-r from-blue-600 to-blue-700 text-white p-4 rounded-lg mb-4 
        ...
        <div className="container mx-auto px-2 sm:px-4">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-lg">Ospiti da assegnare</h3>
                <div className="flex gap-3 text-sm text-blue-100">
                  <span>
                    Adulti: {getUnassignedGuests().adults}/{totalGuests.adults}
                  </span>
                  <span>
                    Bambini: {getUnassignedGuests().children}/
            ...
            <div className="text-right">
              <div className="text-3xl font-bold">{getTotalUnassigned()}</div>
              <div className="text-xs text-blue-100">rimanenti</div>
            </div>
          </div>
        </div>
      </div>
...
function BookingPageContent() {
  ...
  const [availabilityByNight, setAvailabilityByNight] =
    useState<SearchResponse["availabilityByNight"]>();
  const [guestTypes, setGuestTypes] = useState<GuestType[]>([]);
  // Add this state to track room privacy costs
  const [roomPrivacyCosts, setRoomPrivacyCosts] = useState<{
    [roomId: number]: number;
  }>({});
  const [currentView, setCurrentView] = useState<"search" | "checkout">(
    "search",
  );
  ...
  const [allBlockedBeds, setAllBlockedBeds] = useState<{
    [roomId: number]: { [date: string]: number[] };
  }>({});
  const [currentBookingId, setCurrentBookingId] = useState<number | null>(null);
  const serviceWorkerRef = useRef<ServiceWorker | null>(null);
  const searchDatesRef = useRef<{ checkIn?: Date; checkOut?: Date }>({});
  const [isSearching, setIsSearching] = useState(false);
  const [datesChangedAfterSearch, setDatesChangedAfterSearch] = useState(false);
  ...
  const [pricingInfo, setPricingInfo] = useState<{
    bb: Array<{ description: string; price: number; peopleCount: number }>;
    mp: Array<{ description: string; price: number; peopleCount: number }>;
  } | null>(null);
  ...
  useEffect(() => {
    const handleNexiCancel = async () => {
      ...
      console.log(
        "[Home] Detected Nexi checkout cancellation, codTrans:",
        codTrans,
      );
      ...
      try {
        ...
        if (data.success) {
          console.log(
            "[Home] Booking cancelled due to checkout abort:",
            codTrans,
          );
        } else {
  ...
  const handleBlockedBedsChange = useCallback(
    (roomId: number, blockedBedsData: { [date: string]: number[] }) => {
  ...
  useEffect(() => {
    ...
    if (showResults) {
      ...
      // Se non ci sono risultati visualizzati, controlla se le date sono cambiate rispetto all'ultima ricerca
      const lastSearchCheckIn = searchDatesRef.current.checkIn?.getTime();
      const lastSearchCheckOut = searchDatesRef.current.checkOut?.getTime();
      const currentCheckIn = checkIn?.getTime();
      const currentCheckOut = checkOut?.getTime();
      ...
      if (
        lastSearchCheckIn !== undefined &&
        lastSearchCheckOut !== undefined &&
        (lastSearchCheckIn !== currentCheckIn ||
          lastSearchCheckOut !== currentCheckOut)
      ) {
        setDatesChangedAfterSearch(true);
      }
    }
  ...
  // Effetto per caricare le tariffe all'avvio
  useEffect(() => {
  ...
  // Effetto per caricare i giorni bloccati all'avvio
  useEffect(() => {
    const fetchBlockedDays = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const dates = data.blockedDays.map(
  ...
  const handleSearch = async () => {
    ...
    try {
      setIsSearching(true);
      setSearchError(null);
      setDatesChangedAfterSearch(false); // Reset flag when starting a new search
      const guests = [
      ...
      if (data.status === "enough" && data.rooms) {
        ...
        console.log(
          "🔎 Risposta API /api/booking-hold:",
          bookingHoldData,
          "reason:",
          bookingHoldData?.reason,
        );
        ...
        data.status &&
        data.status.startsWith("too_little_availability")
      ) {
  ...
  const handlePrivacyCostChange = useCallback(
    (roomId: number, cost: number) => {
    ...
    <div className="flex items-center justify-between py-4">
      <div>
        <div className="font-medium">
          {type === "adults"
            ? t("booking.adultsLabel", { var: { count: value } })
            : type === "children"
              ? t("booking.childrenLabel", { var: { count: value } })
              : t("booking.infantsLabel", { var: { count: value } })}
        </div>
        <div className="text-sm text-gray-500">
          {type === "adults"
            ? t("booking.adultsSubtitle")
            : type === "children"
              ? t("booking.childrenSubtitle")
              : t("booking.infantsSubtitle")}
        </div>
      </div>
      {type === "infants" ? (
        <Popover>
          ...
          <PopoverContent className="w-80">
            <p className="text-sm">{t("booking.infantsInfo")}</p>
          </PopoverContent>
        </Popover>
      ) : (
        <div className="flex items-center gap-3">
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <Cart
        ...
        bookingDetails={{
          ...
          accommodation:
            pensionType === "bb" ? "Bed & Breakfast" : "Mezza Pensione",
          rooms: rooms.reduce((acc: RoomCartItem[], room) => {
            const roomGuests = assignedGuests.filter(
              (guest) => guest.roomId === room.roomId,
            );
                    ...
                    <PopoverContent className="w-auto p-0" align="start">
                      <CalendarComponent
                        ...
                        disabled={(date) => {
                          ...
                          // Disabilita i giorni bloccati dall'admin
                          const dateStr = date.toISOString().split("T")[0];
                          return blockedDays.some((blockedDate) => {
                            const blockedDateStr = blockedDate
                              .toISOString()
                              .split("T")[0];
                  ...
                  <Popover open={guestsOpen} onOpenChange={setGuestsOpen}>
                    ...
                    <PopoverContent className="w-[280px] sm:w-80">
                      <div className="px-1 divide-y">
                        <GuestCounter value={adults} type="adults" />
                        <GuestCounter value={children} type="children" />
                        <GuestCounter value={infants} type="infants" />
                      </div>
                    </PopoverContent>
                  </Popover>
                </div>
                ...
                <div>
                  <Button
                    ...
                    disabled={
                      ...
                      (!datesChangedAfterSearch &&
                        searchDatesRef.current.checkIn !== undefined)
                    }
            ...
            {!showResults && !isSearching && (
              <>
                <div className="max-w-4xl mx-auto px-2 sm:px-4 py-3">
                  <div
                    className="p-3 sm:p-4  border border-gray-200 rounded-lg text-gray-400 text-sm sm:text-base"
                    dangerouslySetInnerHTML={{
                      __html: t("booking.instructions"),
                    }}
                ...
                {/* Box delle tariffe */}
                <div className="max-w-4xl mx-auto px-2 sm:px-4 py-3">
                  <div className="p-3 sm:p-4  border  rounded-lg text-sm sm:text-base">
                    ...
                    {pricingInfo ? (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <ul className="space-y-1 ">
                            {pricingInfo.mp.map((item, index) => (
                              <li key={index}>
                                {item.description}: €{item.price}{" "}
                                {t("booking.pricingPerNight")}
                              </li>
                            ))}
                          </ul>
                        </div>
                      ...
                      <p className="">Caricamento tariffe...</p>
                    )}

                    <div className="mt-4 pt-3 border-t ">
                      <p className="text- font-medium flex items-start gap-2">
                        <Info className="h-5 w-5 mt-0.5 flex-shrink-0" />
                        <span>{t("booking.infantFreeInfo")}</span>
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="max-w-4xl mx-auto px-2 sm:px-4">
                {searchError === "blocked_days" && (
                  <div className="p-3 sm:p-4 bg-amber-50 border border-amber-200 rounded-lg text-amber-700 flex items-start sm:items-center">
                    <svg
                      ...
                      className="h-5 w-5 mr-2 mt-1 sm:mt-0"
                      viewBox="0 0 20 20"
                      fill="currentColor"
                    >
                      <path
                    ...
                    <span className="text-sm sm:text-base">
                      <strong>{t("searchErrors.noResultsTitle")}</strong>{" "}
                      {t("searchErrors.noResultsContact")}
                    </span>
                  </div>
                )}
                {searchError === "booking_in_progress" && (
                  <div className="p-3 sm:p-4 bg-blue-50 border border-blue-200 rounded-lg text-blue-700 text-sm sm:text-base">
                    <p>{t("searchErrors.bookingInProgress")}</p>
                  </div>
                )}
                {searchError === "sold_out" && (
                  <div className="p-3 sm:p-4 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm sm:text-base">
                    <p>{t("searchErrors.soldOut")}</p>
                  </div>
                )}
                {searchError?.startsWith("too_little_availability") && (
                  <div className="p-3 sm:p-4 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm sm:text-base">
                    <p>{t("searchErrors.tooLittleAvailability")}</p>
                  </div>
                )}
                {["blocked_days", "booking_in_progress", "sold_out"].includes(
                  searchError,
                ) === false &&
                  !searchError?.startsWith("too_little_availability") && (
                    <div className="p-3 sm:p-4 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm sm:text-base">
                      <p>{t("searchErrors.genericError")}</p>
                    </div>
                  )}
              </div>
            )}
              ...
              <>
                    ...
                    <div className="flex-grow">
                      ...
                      >
                        <SelectTrigger className="w-full sm:w-auto">
                          <SelectValue
                            placeholder={t("booking.selectTreatment")}
                          />
                        </SelectTrigger>
                        <SelectContent>
                ...
                <Card className="sm:p-6 max-w-4xl mx-auto border-0 shadow-none sm:border sm:shadow">
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div className="pt-1">
                      <span className="text-sm sm:text-base block font-semibold">
                        {t("booking.step2")}
                      </span>
                    </div>
                  </div>
